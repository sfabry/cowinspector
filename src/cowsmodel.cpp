...
void CowsModel::refresh()
{
    ...
    QSqlQuery query(db);
    query.prepare(QString("with mealdays as (select cow, sum(fooda) as sa, sum(foodb) as sb, date(entry - time '%1') as day from meals group by cow, day order by day desc), "
                  ...
                  "mealtoday.sa as meala, mealtoday.sb as mealb from alloc left outer join mealtoday on (mealtoday.cow = alloc.cow) where (alloc.fooda + alloc.foodb > :minFood) order by cow asc").arg(BoxModel::globalNewDayTime().toString("hh:mm:ss")));
